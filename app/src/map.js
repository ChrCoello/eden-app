...
export function createMap(svg, garden, scan, onSelect) {
  ...
  for (const f of garden.features) {
    ...
    if (kind === "path" || kind === "chemin" || kind === "house") continue;     // not tappable, no label
    shape.dataset.id = id;
    const [x, y] = labelPoint(f.geometry, f.properties.label);
  ...
  return {
    select,
    /** Colour zones and caption their labels: levelOf(id) → dryness level, null for "never
